...
assert.match(html,/id="seasonMatches"/);
assert.match(html,/Alle vier Teams, Termine und Ergebnisse/);
...
assert.match(html,/id="closeLivePushAlert"/);
...
assert.match(script,/\/api\/v1\/darts\/season/);
assert.match(script,/\/api\/v1\/darts\/matches\//);
assert.match(script,/native-match-stats/);
...
assert.match(script,/native-live-scores/);
assert.match(script,/match-detail-tabs/);
assert.match(script,/Spielverlauf/);
...
assert.match(fs.readFileSync('sw.js','utf8'),/client\.postMessage\(\{type:"clubiq-darts-push"/);
...
assert.match(backend,/@app\.get\("\/api\/v1\/darts\/season"\)/);
assert.match(backend,/@app\.get\("\/api\/v1\/darts\/matches\/\{match_id\}"\)/);
