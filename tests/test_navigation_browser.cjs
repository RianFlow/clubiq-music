...
(async () => {
  ...
  try {
    ...
    Object.assign(player,{buffer_phase:'starting',buffer_seconds:4,buffer_start_seconds:10,buffer_refill_seconds:15,buffer_target_seconds:90});
    await page.evaluate(() => loadPlayerState(true));
    await page.locator('#playbackHealthTitle').getByText('Startpuffer wird geladen',{exact:true}).waitFor();
    await page.locator('#playerBufferText').getByText(/Startreserve 10 s/).waitFor();
    assert.equal(await page.locator('#playerBuffer').getAttribute('max'),'90');
    if (process.env.NAV_SCREENSHOT_DIR) await page.locator('.player-console').screenshot({path:path.join(process.env.NAV_SCREENSHOT_DIR,'music-buffer-start.png')});
    player.buffer_phase = 'refilling'; player.buffer_seconds = 8;
    await page.evaluate(() => loadPlayerState(true));
    await page.locator('#playbackHealthTitle').getByText('Musik puffert nach',{exact:true}).waitFor();
    await page.locator('#playerBufferText').getByText(/Weiter ab 15 s/).waitFor();
    assert.equal(await page.locator('#playerPlay').getAttribute('aria-label'),'Pause');
    if (process.env.NAV_SCREENSHOT_DIR) await page.locator('.player-console').screenshot({path:path.join(process.env.NAV_SCREENSHOT_DIR,'music-buffer-refill.png')});
    assert.equal(await page.locator('#playerPlay').isDisabled(),false);
    player.buffering = false; player.playing = true; player.buffer_phase = 'ready';
    await page.evaluate(() => loadPlayerState(true));
